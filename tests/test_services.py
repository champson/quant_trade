...
def test_market_history_continues_and_flushes_after_provider_failure(app_config):
    ...
    class Router:
        def fetch(self, request):
            calls.append(request.end)
            if request.end == days[1]:
                raise EmptyDataError("暂时为空")
            data = _rows("000001.SZ", [pd.Timestamp(request.end)])
            data["adjustment"] = "none"
            return DataBatch(data, "market", request)

    store = DataStore(app_config)
    with pytest.raises(DataQualityError, match="可直接重跑以续传"):
        update_market_history(
            app_config,
            Router(),
            store,
            days,
            include_basic=False,
            on_error=lambda dataset, day, exc: errors.append((dataset, day, str(exc))),
        )

    assert calls == days
    assert errors == [("行情", days[1], "暂时为空")]
    assert store.incomplete_market_snapshot_dates(AssetType.STOCK, days) == [days[1]]
    saved = store.read_daily_dates([], [days[0], days[2]], asset_type=AssetType.STOCK)
    assert set(pd.to_datetime(saved["trade_date"]).dt.date) == {days[0], days[2]}


def test_market_history_flushes_pending_before_unexpected_failure(app_config):
    app_config.providers.market_snapshot_min_symbols = {"stock": 1}
    ...
    with pytest.raises(RuntimeError, match="unexpected"):
        update_market_history(app_config, Router(), store, days, include_basic=False)

    assert store.market_snapshot_complete(AssetType.STOCK, days[0])
...
def test_reference_tolerance_allows_small_historical_universe_shortfall(app_config):
    app_config.providers.market_snapshot_min_symbols = {"stock": 4500}
    app_config.providers.market_snapshot_reference_ratio = 0.9
    app_config.providers.market_snapshot_reference_tolerance_symbols = 10
    day = date(2015, 1, 20)
    symbols = [f"{index:06d}.SZ" for index in range(2332)]
        ...
        def fetch(self, request):
            if request.dataset == Dataset.DAILY_BASIC:
                return DataBatch(
                    pd.DataFrame(
                        {
                            "ts_code": symbols[:2326],
                            "trade_date": ["20150120"] * 2326,
                            "total_mv": [1.0] * 2326,
                        }
                    ),
                    "history",
                    request,
                    metadata={"expected_symbols": 2593},
                )
            data = pd.concat(
                [_rows(symbol, [pd.Timestamp(day)]) for symbol in symbols],
                ignore_index=True,
            )
            data["adjustment"] = str(request.adjustment)
            return DataBatch(
    ...
    update_daily_basic(
        ...
        store,
        day,
        reference_ratio=app_config.providers.market_snapshot_reference_ratio,
        reference_tolerance_symbols=(
            app_config.providers.market_snapshot_reference_tolerance_symbols
        ),
    )
    update_bars(app_config, router, store, [], day, day, AssetType.STOCK)
    ...
    with store.connect() as con:
        assert con.execute(
            "SELECT expected_symbols FROM market_snapshots WHERE trade_date = ?", [day]
        ).fetchone() == (2323,)


def test_bounded_full_market_response_uses_daily_basic_instead_of_listed_universe(
    app_config,
):
    day = date(2015, 3, 16)
    basic_symbols = [f"{index:06d}.SZ" for index in range(20)]
    market_symbols = basic_symbols + ["000020.SZ", "000021.SZ"]
    ...
    class Router:
        def fetch(self, request):
            metadata = {
                ...
                "full_market_response_rows": len(
                    basic_symbols if request.dataset == Dataset.DAILY_BASIC else market_symbols
                ),
            ...
            if request.dataset == Dataset.DAILY_BASIC:
                return DataBatch(
                    pd.DataFrame(
                        {
                            "ts_code": basic_symbols,
                            "trade_date": ["20150316"] * len(basic_symbols),
                            "total_mv": [1.0] * len(basic_symbols),
                        }
                    ),
                    "history",
                    request,
            ...
            data = pd.concat(
                [_rows(symbol, [pd.Timestamp(day)]) for symbol in market_symbols],
                ignore_index=True,
            )
            data["adjustment"] = str(request.adjustment)
            return DataBatch(data, "history", request, metadata=metadata)
...
def test_full_market_response_at_source_limit_is_incomplete(app_config):
    app_config.providers.market_snapshot_reference_tolerance_symbols = 0
    day = date(2024, 1, 8)
    ...
    class Router:
        def fetch(self, request):
            ...
            return DataBatch(
                ...
                metadata={
                    "expected_symbols": 1,
                    "full_market_response_complete": False,
                    "full_market_response_rows": 6000,
                    "full_market_response_limit": 6000,
                },
            )

    store = DataStore(app_config)
    with pytest.raises(DataQualityError, match="至少需要 6001"):
        update_bars(app_config, Router(), store, [], day, day, AssetType.STOCK)
    assert not store.market_snapshot_complete(AssetType.STOCK, day)


def test_bounded_full_market_response_must_cover_daily_basic_symbols(app_config):
    day = date(2024, 1, 8)
    store = DataStore(app_config)
    store.write_daily_basic(
        pd.DataFrame(
    ...
    store.mark_daily_basic_snapshot(
        ...
        provider="basic",
        status="complete",
        details={"symbol_digest": store.symbol_digest({"000001.SZ", "000002.SZ"})},
    )
    ...
    class Router:
        def fetch(self, request):
            data = pd.concat(
                [
                    _rows("000001.SZ", [pd.Timestamp(day)]),
                    _rows("000003.SZ", [pd.Timestamp(day)]),
                ],
                ignore_index=True,
            )
            data["adjustment"] = str(request.adjustment)
...
def test_signal_refuses_partial_configured_universe_on_target_date(app_config):
    app_config.strategies = {
        "logbias": {
            "enabled": True,
            "asset_type": "etf",
            "symbols": ["510300.SH", "512800.SH"],
            "ema_window": 2,
        }
    }
    store = DataStore(app_config)
    store.write_daily(
        pd.concat(
            [
                _rows("510300.SH", pd.to_datetime(["2024-01-09", "2024-01-10"])),
                _rows("512800.SH", pd.to_datetime(["2024-01-09"])),
            ]
        ),
        AssetType.ETF,
    )

    with pytest.raises(DataQualityError, match="512800.SH.*部分证券缺失"):
        run_strategy_signal(app_config, store, "logbias", "2024-01-10")
    assert not (app_config.paths.artifacts_dir / "signals" / "logbias").exists()


def test_signal_refuses_to_publish_without_sufficient_indicator_history(app_config):
    app_config.strategies = {
        "logbias": {
            "enabled": True,
            "asset_type": "etf",
    ...
    store = DataStore(app_config)
    store.write_daily(
        _rows("510300.SH", pd.bdate_range("2024-01-02", periods=3)),
        AssetType.ETF,
    )

    with pytest.raises(DataQualityError, match="信号历史不足.*510300.SH=3"):
        run_strategy_signal(app_config, store, "logbias", "2024-01-04")

    assert not (app_config.paths.artifacts_dir / "signals" / "logbias").exists()


def test_backtest_uses_pre_start_history_for_indicator_warmup(app_config):
    app_config.strategies = {
        "logbias": {
            "enabled": True,
            "asset_type": "etf",
    ...
    dates = pd.bdate_range("2023-12-11", "2024-01-10")
    dates = dates[dates != pd.Timestamp("2024-01-01")]
    bars = _rows("510300.SH", dates)
    before = bars["trade_date"] < pd.Timestamp("2023-12-29")
    bars.loc[before, ["open", "high", "low", "close"]] = [10.0, 10.0, 10.0, 10.0]
    bars.loc[~before, ["open", "high", "low", "close"]] = [11.0, 11.0, 11.0, 11.0]
    store = DataStore(app_config)
    store.write_daily(bars, AssetType.ETF)

    result = run_strategy_backtest(app_config, store, "logbias", "2024-01-02", "2024-01-10")

    assert result.equity.index.min() == pd.Timestamp("2024-01-02")
    assert result.trades.iloc[0]["signal_date"] == pd.Timestamp("2023-12-29")
    assert result.trades.iloc[0]["date"] == pd.Timestamp("2024-01-02")
...
def test_backtest_runs_are_versioned_instead_of_overwritten(app_config):
    ...
    store = DataStore(app_config)
    bars = _rows("510300.SH", pd.bdate_range("2023-12-11", "2024-01-12"))
    store.write_daily(bars, AssetType.ETF)
...
def test_backtest_refuses_to_run_without_pre_start_warmup(app_config):
    app_config.strategies = {
        "logbias": {
            "enabled": True,
            "asset_type": "etf",
    ...
    store = DataStore(app_config)
    store.write_daily(
        _rows("510300.SH", pd.bdate_range("2024-01-02", "2024-01-12")),
        AssetType.ETF,
    )

    with pytest.raises(DataQualityError, match="预热历史不足.*510300.SH=0"):
        run_strategy_backtest(app_config, store, "logbias", "2024-01-02", "2024-01-12")

    assert not (app_config.paths.artifacts_dir / "backtests").exists()


def test_backtest_omits_incomplete_benchmark_comparison(app_config):
    app_config.strategies = {
        "logbias": {
            "enabled": True,
            "asset_type": "etf",
    ...
    store = DataStore(app_config)
    dates = pd.bdate_range("2023-12-11", "2024-01-12")
    store.write_daily(_rows("510300.SH", dates), AssetType.ETF)
    store.write_daily(
        _rows("000300.SH", pd.bdate_range("2024-01-05", "2024-01-12")),
        AssetType.INDEX,
    )

    result = run_strategy_backtest(app_config, store, "logbias", "2024-01-02", "2024-01-12")
    metrics = json.loads(Path(result.artifacts["metrics"]).read_text(encoding="utf-8"))

    assert metrics["benchmark"] == {}
    assert metrics["benchmark_period"] is None
    assert "区间不完整" in metrics["benchmark_status"]
