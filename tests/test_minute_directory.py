...
@pytest.mark.parametrize("relative_file", ["../outside.csv", "/tmp/outside.csv"])
def test_manifest_rejects_paths_outside_import_root(app_config, tmp_path, relative_file):
    source = tmp_path / "source"
    source.mkdir()
    pd.DataFrame(
        [
    ...
    with pytest.raises(DataQualityError, match="相对路径|不得越过"):
        MinuteDirectoryImporter(app_config, DataStore(app_config)).discover(source)
...
def test_directory_import_rejects_non_whole_minute_timestamp(app_config, tmp_path):
    source = tmp_path / "source" / "stock"
    source.mkdir(parents=True)
    (source / "000001.SZ.csv").write_text(
        HEADER + "000001.SZ,2024-01-02 09:35:30,10,10,10,10,100,1000\n",
        encoding="utf-8-sig",
    )
    ...
    assert result.status == "partial_failed"
    assert "不符合5min交易时段" in result.failures[0]["error"]
