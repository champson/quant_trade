...
def test_zip_filters_tushare_etf_opening_placeholder_before_grid_validation(app_config):
    app_config.ensure_directories()
    path = app_config.minute.inbox / "etf-placeholder.zip"
    csv = """ts_code,trade_time,open,high,low,close,vol,amount
510300.SH,2024-01-02 09:31:00,1,1,1,1,0,0
510300.SH,2024-01-02 09:35:00,4,4.1,3.9,4,100,400
"""
    with zipfile.ZipFile(path, "w") as archive:
        archive.writestr("510300.SH.csv", csv)
    store = DataStore(app_config)
    ...
    assert result.status == "success"
    data = store.read_minute(["510300.SH"], "2024-01-02", "2024-01-03", "5min")
    assert data["bar_time"].dt.strftime("%H:%M:%S").tolist() == ["09:35:00"]


def test_zip_rejects_non_whole_minute_timestamp(app_config):
    app_config.ensure_directories()
    path = app_config.minute.inbox / "seconds.zip"
    csv = """ts_code,trade_time,open,high,low,close,vol,amount
000001.SZ,2024-01-02 09:31:30,10,10,10,10,100,1000
"""
    with zipfile.ZipFile(path, "w") as archive:
        archive.writestr("000001.SZ.csv", csv)

    with pytest.raises(DataQualityError, match="不符合1min交易时段"):
        MinuteArchiveImporter(app_config, DataStore(app_config)).import_archive(path)
