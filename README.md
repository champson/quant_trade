...
完整生成后才原子发布，不会留下可见的半成品，也不会覆盖此前结果。指标会读取并验证
`--start` 之前的预热历史，但净值和绩效仍从起始日计算；起始日前最后一个收盘信号可在
首日开盘成交。基准只有完整覆盖策略全部交易日时才计算比较指标，否则报告明确记录缺口。
`qt dashboard` 按数据日期选择最新复盘和信号，并兼容旧版直接位于策略目录的报告。
...
manifest 的 `relative_file` 必须是导入根目录内的相对路径，绝对路径、`..` 越界和符号链接
越界都会被拒绝。
...
`bar_start` 会按频率平移为统一的结束时间。ZIP 和目录导入都要求时间戳落在整分钟及对应
频率网格。`qt data minute verify` 会深检目录元数据、键、时间范围和 OHLCV，而不只检查
文件存在。5分钟数据可以向上聚合，不能用来生成真实1分钟行情。
