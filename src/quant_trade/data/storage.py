...
class DataStore:
    ...
    def completed_market_snapshot_dates(
        self,
        asset_type: AssetType | str,
        adjustment: Adjustment | str = Adjustment.NONE,
        *,
        ...
        """Return dates carrying a complete marker; callers validate dates they consume."""
        clauses = ["asset_type = ?", "adjustment = ?", "status = 'complete'"]
        params: list[Any] = [AssetType(asset_type).value, Adjustment(adjustment).value]
        if start is not None:
            clauses.append("trade_date >= ?")
            params.append(pd.Timestamp(start).date())
        if end is not None:
        ...
        with self.connect() as con:
            rows = con.execute(
                "SELECT trade_date FROM market_snapshots WHERE "
                + " AND ".join(clauses)
                + " ORDER BY trade_date",
                params,
            ).fetchall()
    ...
    def read_daily(
        ...
        *,
        ...
        if start:
            clauses.append("CAST(trade_date AS DATE) >= ?")
            params.append(pd.Timestamp(start).date())
        if end:
            clauses.append("CAST(trade_date AS DATE) <= ?")
            params.append(pd.Timestamp(end).date())
