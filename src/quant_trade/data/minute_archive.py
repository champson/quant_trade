...
def minute_placeholder_mask(frame: pd.DataFrame) -> pd.Series:
    """Identify Tushare's synthetic ETF opening row."""
    if frame.empty:
    ...
    return frame["asset_type"].eq("etf") & clock & one_price & zero_trade


def validate_minute_time_grid(frame: pd.DataFrame, frequency: str, asset_type: str) -> None:
    """Require canonical whole-minute bar-end timestamps inside exchange sessions."""
...
class MinuteArchiveImporter:
    ...
    def _normalize_chunk(
        ...
        out = out.loc[~minute_placeholder_mask(out)].copy()
    ...
    def _validate_time_grid(frame: pd.DataFrame, frequency: str, asset_type: str) -> None:
        validate_minute_time_grid(frame, frequency, asset_type)
