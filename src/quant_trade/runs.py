...
class RunTracker:
    ...
    def finish(self, status: str, details: dict | None = None) -> None:
        details = details or {}
        payload = {
            "run_id": self.run_id, "task": self.task, "as_of": self.as_of,
            "started_at": self.started.isoformat(), "finished_at": datetime.now().isoformat(),
        ...
        with self.store.connect() as con:
            ...
            con.execute(
                ...
                [self.run_id, self.task, self.started, datetime.now(), status, self.as_of,
