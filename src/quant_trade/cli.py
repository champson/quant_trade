...
from quant_trade.pipelines.daily import _anchors, run_daily
...
def review_close(
    ...
    available_dates = store.completed_market_snapshot_dates(AssetType.STOCK, end=target_day)
    try:
        snapshot_dates = _anchors(available_dates, target_day)
    except ValueError as exc:
        raise typer.BadParameter(str(exc), param_hint="--as-of") from exc
    incomplete = store.incomplete_market_snapshot_dates(AssetType.STOCK, snapshot_dates)
    if incomplete:
        raise typer.BadParameter(
            "复盘目标日或收益锚点缺少完整全市场快照: " + ", ".join(str(day) for day in incomplete),
            param_hint="--as-of",
        )
    bars = store.read_daily_dates([], snapshot_dates, asset_type=AssetType.STOCK, adjustment="none")
    if bars.empty:
